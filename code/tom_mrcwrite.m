...
%   4   float   x_stage  Stage x position (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   y_stage  Stage y position (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   z_stage  Stage z position (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   x_shift  Image shift x (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   y_shift  Image shift y (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   z_shift  Image shift z (Unit=m. But if value>1, unit=ï¿½m)
%   4   float   defocus  Defocus Unit=m. But if value>1, unit=ï¿½m)
...
MRC.my=MRC.ny;
MRC.mz=MRC.nz;
MRC.xlen=MRC.nx*pixelsize;
MRC.ylen=MRC.ny*pixelsize;
MRC.zlen=MRC.nz*pixelsize;
...
for lauf=1:1024
    ...
    fwrite(fid,0,'float');                      %float: 4 bytes
    fwrite(fid,Data.Header.Tiltaxis,'float');   %float: 4 bytes
    fwrite(fid,pixelsize,'float');  %float: 4 bytes
